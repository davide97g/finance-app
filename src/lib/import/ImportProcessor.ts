...
import { ParsedData, PotentialMerge, RecurringConflict } from "./types";
...
export class ImportProcessor {
  ...
  async process(
    ...
    mergedCategoryIds?: Map<string, string>,
    ...
    if (data.source === "legacy_vue") {
      ...
      return this.processStandardImport(
        data,
        onProgress,
        mergedCategoryIds,
        skippedRecurringIds
  ...
  async analyzeCategoryConflicts(data: ParsedData): Promise<PotentialMerge[]> {
    ...
    for (const importedCat of data.categories) {
      ...
      if (bestMatch) {
        console.log(
          `Checking '${importedName}': Best match '${bestMatch.match}' (dist: ${bestMatch.distance})`
        );
      } else {
  ...
  async loadExistingRecurring() {
    this.existingRecurring = await db.recurring_transactions
      .where("user_id")
      .equals(this.userId)
      .toArray();
  }
  ...
  analyzeRecurringConflicts(data: ParsedData): RecurringConflict[] {
    ...
    if (!this.existingRecurring || this.existingRecurring.length === 0)
      return [];
    ...
    for (const importedRec of data.recurring) {
      ...
      if (match) {
        ...
        conflicts.push({
          imported: {
            ...importedRec,
            name: importedRec.description,
            id: importedRec.id || uuidv4(),
          },
          existing: {
            id: match.id,
  ...
  analyzeGroupData(data: ParsedData): {
    hasGroups: boolean;
    ...
    if (data.transactions) {
      groupTransactionCount = data.transactions.filter((t) => {
        ...
        const raw =
          (t.raw_data as { groupId?: string; group_id?: string }) || {};
  ...
  private async processStandardImport(
    data: ParsedData,
    onProgress?: ImportProgressCallback,
    mergedCategoryIds?: Map<string, string>,
    skippedRecurringIds?: Set<string>
    ...
    // CONTEXTS LOOKUP
    const existingContexts = await db.contexts
      .where("user_id")
      .equals(this.userId)
      .toArray();
    const existingContextsMap = new Map(
      existingContexts.map((c) => [c.name, c.id])
    );
    ...
    if (data.categories) {
      ...
      for (const cat of data.categories) {
        ...
        const isExisting = Array.from(existingCategoriesMap.values()).includes(
          finalId
        );
        ...
        if (!isExisting && !isQueued) {
          categoriesToInsert.push({
            ...
            parent_id: cat.parent_id
              ? categoryIdMap.get(cat.parent_id)
              : undefined,
    ...
    if (data.transactions) {
      ...
      for (const tx of data.transactions) {
        ...
        let finalCatId = tx.category_id
          ? categoryIdMap.get(tx.category_id)
          : undefined;
        if (!finalCatId) {
          // Check if the id is ALREADY a valid category ID (manual categorization)
          const isManualId = Array.from(
            existingCategoriesMap.values()
          ).includes(tx.category_id || "");
          if (isManualId) {
        ...
        const finalCtxId = tx.context_id
          ? contextIdMap.get(tx.context_id!)
          : undefined;
        ...
        if (tx.group_id && data.groups && data.group_members) {
          const groupMembers = (data.group_members || []).filter(
            (m) => m.group_id === tx.group_id
          );
          const myMemberRecord = groupMembers.find(
            (m) => m.user_id === tx.user_id
          );
    ...
    if (data.recurring) {
      ...
      for (const rec of data.recurring) {
        ...
        const mappedId = rec.category_id
          ? categoryIdMap.get(rec.category_id)
          : undefined;
        let finalCatId = mappedId;
        if (!finalCatId) {
          finalCatId = UNCATEGORIZED_CATEGORY.ID;
        }
        const finalCtxId = rec.context_id
          ? contextIdMap.get(rec.context_id)
          : undefined;
    ...
    if (data.budgets) {
      ...
      const existingBudgets = await db.category_budgets
        .where("user_id")
        .equals(this.userId)
        .toArray();
      // Map key: category_id + period
      const existingBudgetsSet = new Set(
        existingBudgets.map((b) => `${b.category_id}_${b.period}`)
      );
    ...
    await db.transaction(
      ...
      async () => {
        if (contextsToInsert.length)
          await db.contexts.bulkPut(contextsToInsert);
        ...
        if (recurringToInsert.length)
          await db.recurring_transactions.bulkPut(recurringToInsert);
        if (budgetsToInsert.length)
          await db.category_budgets.bulkPut(budgetsToInsert);
      }
    );
    ...
    return {
      ...
      orphanCount,
      skippedCount: (data.transactions?.length || 0) - importedTransactions,
    };
  }

  // --- VUE MIGRATION STRATEGY ---
  private async processVueImport(
    data: ParsedData,
    onProgress?: ImportProgressCallback,
    mergedCategoryIds?: Map<string, string>,
    skippedRecurringIds?: Set<string>,
    ...
    const ROOT_CATEGORY_TYPES: Record<
      string,
    ...
    if (data.categories) {
      ...
      for (const vueCat of data.categories) {
        ...
        const existingId = existingCategoriesMap.get(
          (vueCat.title || "").toLowerCase()
        );
        if (existingId) {
      ...
      for (const vueCat of data.categories) {
        ...
        const parentId =
          vueCat.parentCategoryId || vueCat.parentId || vueCat.parent_id;
        ...
        if (options?.regenerateColors) {
          categoryColor = generateSemanticColor(
            categoryType,
    ...
    if (data.recurring) {
      ...
      for (const vueRec of data.recurring) {
        ...
        recurringToInsert.push({
          ...
          start_date: (vueRec.nextOccurrence || vueRec.startDate || "").split(
            "T"
          )[0],
